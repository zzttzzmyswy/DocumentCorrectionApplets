...
def baiduOCR(filPath,name):
    ...
    file_handle = open(os.path.dirname(filPath)+'/' +
                       os.path.basename(filPath).split('.')[0]+'.txt', mode='w',encoding="utf-8")
    file_handle.write(filPath+' 文字识别内容:\r\n')
    file_handle.write("识别准确率（仅供参考）:"+str(round(retProbability*100,2))+'%\r\n')
    print(name+"识别准确率（仅供参考）:"+str(round(retProbability*100,2))+'%')
...
def fContours(img, edged):
    ...
    for c in cnts:
        ...
        if len(approx) == 4:
            ...
            print("边缘检测预估文档面积："+str(cont))
            if cont < 200000:
                print("边缘检测预估文档面积过小，轮廓识别梯形矫正结果可能不正确")
                # 当预估面积过小时抛弃此次运算
                # screenCnt = None
...
def documentCorrection(file_path):  # 文档图像处理主函数
    ...
    srcy = cv2.resize(src, (1200, int(heigh)), fx=None,
                      fy=None, interpolation=cv2.INTER_AREA)
    gray = cv2.cvtColor(srcy, cv2.COLOR_BGR2GRAY)
    saveIntermediateImage(
        "2.缩放后的原图像", src, fileName)
    gray = cv2.GaussianBlur(gray, (3, 3), 0)
    saveIntermediateImage(
        "3.灰度图像", gray, fileName)
    # 生成腐蚀和膨胀所需的核
    kernel = np.ones((5, 5), np.uint8)
    # 腐蚀：取结构元所指定的领域内值的最小值作为该位置的输出灰度值
    etchImage = cv2.erode(gray, kernel)
    saveIntermediateImage("4.腐蚀后图像", etchImage,
                          fileName)
    # 膨胀：取结构元所指定的领域内值的最大值作为该位置的输出灰度值
    expandImage = cv2.dilate(etchImage, kernel)
    saveIntermediateImage("5.腐蚀并膨胀后图像", expandImage,
                          fileName)
    # 边缘检测
    ...
    if contoursp is not None:
        ...
        if traimg is not None:
            ...
            print(file_path + "文件没有正确地轮廓识别")
    else:
        print(file_path + "文件没有正确地轮廓识别")
    # 霍夫变换得到线条
    lines = cv2.HoughLinesP(cannyImage, 0.9, np.pi / 180, 90,
    ...
    for line in lines:
        ...
        theras.append(thera)
        cv2.line(HoughTransformImage1, (x1, y1), (x2, y2),
    ...
    if lowerLimit < -30:
        lowerLimit = -30
        print("偏移角度分析低阈值超出预定范围-30度，旋转矫正结果可能不正确")
    if upperLimit > 30:
        upperLimit = 30
        print("偏移角度分析高阈值超出预定范围30度，旋转矫正结果可能不正确")
    theras.clear()
    print("自动确认正常角度范围: "+str(round(lowerLimit, 2)) +
          "度"+" - "+str(round(upperLimit, 2))+"度")
    ...
    for line in lines:
        ...
        if thera > lowerLimit and thera < upperLimit:
            # 绘制计算需要的霍夫线
            cv2.line(HoughTransformImage2, (x1, y1), (x2, y2),
                     (255, 255, 255), 1, lineType=cv2.LINE_AA)
            theras.append(thera)
    saveIntermediateImage("8.霍夫变换得到线条图像", HoughTransformImage1,
                          fileName)
    saveIntermediateImage("9.经过自动阈值筛选的霍夫变换得到线条图像", HoughTransformImage2,
                          fileName)
    # 对经过处理的线的角度进行求均值
    thera = np.mean(theras)
    print("旋转角度：" + ("逆时针" if thera > 0 else "顺时针 ")+str(round(thera, 2))+" 度")
    # 使用求得的偏移角度均值来对图像进行旋转
...
if __name__ == "__main__":
    ...
    print("文档图片文件处理完毕，处理结果在optupt文件夹下，处理中间产物在temp文件夹下")
